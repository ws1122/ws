...
const request = (url, options, baseUrl, isLoading) => {
  return new Promise((resolve, reject) => {
    ...
    axios({
      ...
      if (data.error) {
        let rout = getHref.getRouter()
        let fromType = ''
        let id = ''
        if (rout === '#/operate') {
          fromType = getHref.getQuery('fromType')
          id = getHref.getQuery('id')
        }
        if (data.error.code === 401) { // token失效
          ...
          axios({
            ...
            if (res.error) {
              ...
              localStorage.clear()
              if (rout === '#/operate') {
                location.href = isTest ? `https://test.p.cheyuu.com/html/manager/index.html#/operate?id=${id}&fromType=${fromType}&v=${v}` : `https://p.cheyuu.com/html/manager/index.html#/operate?id=${id}&fromType=${fromType}&v=${v}`
              } else {
                location.href = isTest ? 'https://test.p.cheyuu.com/html/manager/index.html#/?v=' + v : '//p.cheyuu.com/html/manager/index.html#/?v=' + v
              }
            } else {
