    ...
    axios({
      ...
      if (data.error) {
        if (data.error.code === 401) { // token失效
          ...
          let licensePlate = getHref.getQuery('licensePlate')
          let randomCode = getHref.getQuery('cheyoo_random_code')
          let pay = getHref.getQuery('pay')
          let isBing = getHref.getQuery('isBing')
          ...
          axios({
            ...
            if (res.error) {
              ...
              localStorage.clear()
              location.href = isTest ? `https://test.m.cheyuu.com/html/bindcarnum/index.html#/?licensePlate=${licensePlate}&contactID=${contactID}&pay=${pay}&cheyoo_random_code=${randomCode}&isBing=${isBing}&v=${timestamp}` : `https://m.cheyuu.com/html/bindcarnum/index.html#/?licensePlate=${licensePlate}&contactID=${contactID}&pay=${pay}&cheyoo_random_code=${randomCode}&isBing=${isBing}&v=${timestamp}`
            } else {
