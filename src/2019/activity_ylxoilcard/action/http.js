    ...
    axios({
      ...
      if (data.error) {
        if (data.error.code === 401) { // token失效
          let oilcardid = getHref.getQuery('oilcardid')
          let merchantid = getHref.getQuery('merchantid')
          let ylxuid = getHref.getQuery('ylxuid')
          ...
          let config = {
            client_id: 'wechat_ylx', // 商家端参数 wechat_partner_app
          ...
          axios({
            ...
            if (res.error) {
              ...
              localStorage.clear()
              location.href = isTest ? `https://test.m.cheyuu.com/html/ylxoilcard/index.html#/?oilcardid=${oilcardid}&merchantid=${merchantid}&ylxuid=${ylxuid}&v=${v}` : `//m.cheyuu.com/html/ylxoilcard/index.html#/?oilcardid=${oilcardid}&merchantid=${merchantid}&ylxuid=${ylxuid}&v=${v}`
            } else {
              if (res.data.scope == null || (res.data.scope).indexOf('member') === -1) { // 还是未授权
                store.dispatch('setAccessToken', '')
                store.dispatch('setRefreshToken', '')
                localStorage.clear()
                location.href = isTest ? `https://test.m.cheyuu.com/html/ylxoilcard/index.html#/?oilcardid=${oilcardid}&merchantid=${merchantid}&ylxuid=${merchantid}&v=${v}` : `//m.cheyuu.com/html/ylxoilcard/index.html#/?oilcardid=${oilcardid}&merchantid=${merchantid}&ylxuid=${ylxuid}&v=${v}`
              } else {
